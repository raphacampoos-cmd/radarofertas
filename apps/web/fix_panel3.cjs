...
// Find where WhatsAppPanel ends.
// The unique string inside WhatsAppPanel before the end is "WHATSAPP_GROUP_ID"
const marker = 'WHATSAPP_GROUP_ID</strong>.';
const indexMarker = content.indexOf(marker);

// find the "}\n" after that marker which is the end of the function.
const endOfWhatsApp = content.indexOf('  )\n}', indexMarker);
if (endOfWhatsApp !== -1) {
    // Cut it off correctly!
    content = content.substring(0, endOfWhatsApp + 5);
}
